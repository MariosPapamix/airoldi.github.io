...
html_2_arx <- html_1 %>%
  ...
  map(function(x){
    x <- x %>%
      glue_data('<li><table><tr><td style="vertical-align: baseline;"><p><a href="https://scholar.google.com/scholar?oi=bibs&cluster={cid}&btnI=1&hl=en" target="_blank">{title}</a> &nbsp;&nbsp;&nbsp;<br>{journal}, {number}, {year}<br>By {author}</p></td><td style="vertical-align: baseline;"><p>({cites} cit.)</p></td></tr></table></li>') %>%
...
html_2_res <- html_1 %>%
  ...
  map(function(x){
    x <- x %>%
      glue_data('<li><table><tr><td style="vertical-align: baseline;"><p><a href="https://scholar.google.com/scholar?oi=bibs&cluster={cid}&btnI=1&hl=en" target="_blank">{title}</a>, {journal}, {year}.</p><td style="vertical-align: baseline;"><p>({cites} cit.)</p></td></tr></table></li>') %>%
      str_replace_all("(, )+</li>", "</li>") %>%
      str_replace_all(", ,", ",") %>%
      char2html() %>%
    return(x);
